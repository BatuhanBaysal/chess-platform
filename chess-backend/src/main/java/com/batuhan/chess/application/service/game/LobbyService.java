...
public class LobbyService {
    ...
    public LobbyService(@Lazy GameService gameService,
    ...
    public void cancelRoom(String roomId, Long userId) {
        RLock lock = redissonClient.getLock("lock:lobby:" + roomId);
        try {
            if (!lock.tryLock(5, 10, TimeUnit.MILLISECONDS)) {
                throw new GameOperationException("Could not acquire lock for room cancellation.");
            }
            try {
            ...
            throw new GameOperationException("Interrupted while waiting for lock", e);
        }
    }
    ...
    public void joinRoom(String roomId, Long userId, String username) {
        ...
        try {
            if (!lock.tryLock(5, 10, TimeUnit.SECONDS)) {
                throw new GameOperationException("Could not acquire lock for joining room.");
            }
            try {
                activeRooms.compute(roomId, (id, room) -> {
