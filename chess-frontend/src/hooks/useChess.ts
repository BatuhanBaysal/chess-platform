...
export const useChess = () => {
    ...
    const client = new Client({
      ...
      onConnect: () => {
        ...
        client.subscribe(`/topic/lobby/${roomId}`, (message) => {
          ...
          if (data.status === 'START_GAME' || data.gameId) {
  ...
  const startNewGame = useCallback(async (existingGameId?: string) => {
    try {
      ...
      const url = existingGameId 
        ? `${API_URL}/games/${existingGameId}?userId=${userId}`
        : `${API_URL}/games?userId=${userId}&whiteId=${userId}`;

      const res = await fetch(url, {
        method: existingGameId ? 'GET' : 'POST',
